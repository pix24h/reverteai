...
const AgentInterface = ({ user }) => {
  ...
  const classificarPressao = (sistolica, diastolica) => {
    ...
    if (sistolica < 120 && diastolica < 80) return { categoria: 'Normal', cor: 'green' };
    if (sistolica < 130 && diastolica < 80) return { categoria: 'Elevada', cor: 'yellow' };
    if (sistolica < 140 || diastolica < 90) return { categoria: 'Hipertensão I', cor: 'orange' };
    if (sistolica < 180 || diastolica < 120) return { categoria: 'Hipertensão II', cor: 'red' };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response.funcoes_executadas && response.funcoes_executadas.length > 0) {
  ...
  const sendAudioMessage = async (audioBlob) => {
    ...
    try {
      ...
      await playTextAsAudio('Desculpe, não consegui processar o áudio. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      // Usar o endpoint de conversa com imagem via GPT-4 Vision
      const token = localStorage.getItem('token');
      const formData = new FormData();
      formData.append('image', file); // Corrigir para 'image' em vez de 'audio'
      formData.append('mensagem', 'Veja esta imagem dos meus ingredientes e me sugira uma receita');
      formData.append('tipo_interacao', 'conversa_geral');
      ...
      if (result.funcoes_executadas && result.funcoes_executadas.length > 0) {
        result.funcoes_executadas.forEach(funcao => {
          if (funcao.function === 'gerar_receita' && funcao.result.success && funcao.result.receita) {
            setCurrentRecipe(funcao.result.receita);
            setShowDashboard(false);
          }
        });
      ...
      // Reproduzir mensagem de erro em áudio
      await playTextAsAudio('Desculpe, não consegui processar a imagem. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getMinimizedIngredients = (ingredientes) => {
    return ingredientes.map(ing => {
      ...
      return ing
        .replace(/^\d+\s*(colher|xícara|ml|g|kg|unidade|dente|pitada|a gosto)?\s*(de\s*)?/i, '')
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-teal-400 to-teal-600 flex flex-col">
      <div className="flex-grow overflow-y-auto p-4">
        ...
        {currentRecipe && (
          <div className="bg-white rounded-2xl p-6 mb-4 shadow-lg">
            ...
            <div className="mb-6">
              <h4 className="text-lg font-semibold text-gray-600 mb-3">Ingredientes necessários:</h4>
              <div className="flex flex-wrap gap-2">
                {getMinimizedIngredients(currentRecipe.ingredientes).map((ing, i) => (
                  <span key={i} className="bg-teal-100 text-teal-800 px-3 py-2 rounded-full text-sm font-medium">
                    {ing}
                  </span>
                ))}
              </div>
            ...
            {showRecipeExpanded && (
              <div className="border-t pt-6 mb-6">
                <div className="space-y-6">
                  <div>
                    <h4 className="font-bold mb-3 text-lg">Ingredientes Completos:</h4>
                  ...
                  <div>
                    <h4 className="font-bold mb-3 text-lg">Modo de Preparo:</h4>
                    <p className="whitespace-pre-line text-base leading-relaxed">{currentRecipe.modo_preparo}</p>
                  </div>
                  
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="font-bold mb-3 text-lg">Informações Nutricionais:</h4>
                    <div className="grid grid-cols-2 gap-3 text-base">
                      <div>Calorias: {currentRecipe.calorias_por_porcao} kcal</div>
                      <div>Carboidratos: {currentRecipe.carboidratos_por_porcao}g</div>
                      <div>Proteínas: {currentRecipe.proteinas_por_porcao}g</div>
                      <div>IG: {currentRecipe.indice_glicemico}</div>
                    </div>
                  </div>
                  ...
                  <button 
                    onClick={() => playTextAsAudio(`${currentRecipe.nome_receita}. Ingredientes: ${currentRecipe.ingredientes.join(', ')}. Modo de preparo: ${currentRecipe.modo_preparo}`)}
              ...
              <div className="flex gap-4">
                ...
                >
                  <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={showRecipeExpanded ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"} />
                  </svg>
        ...
        {showDashboard && (
          <div className="bg-white rounded-2xl p-6 mb-4 shadow-lg">
            <h3 className="text-2xl font-bold text-gray-800 text-center mb-6">Dashboard de Progresso</h3>
            
            {/* Gráfico de Avaliação Visual */}
            {healthData && healthData.saude && healthData.saude.length > 0 && (
              <div className="mb-6">
                <h4 className="text-lg font-bold text-gray-700 mb-4 text-center">Evolução dos Últimos 7 Dias</h4>
                <div className="bg-gradient-to-r from-blue-50 to-green-50 p-4 rounded-lg">
                  {/* Gráfico de Peso */}
                  {healthData.saude.some(d => d.peso) && (
                    <div className="mb-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-gray-600">Peso (kg)</span>
                        <span className="text-sm text-blue-600 font-bold">
                          {healthData.saude.find(d => d.peso)?.peso}kg
                        </span>
                      </div>
                      <div className="flex gap-1 h-8">
                        {healthData.saude.slice(0, 7).reverse().map((data, index) => {
                          const peso = data.peso;
                          const maxPeso = Math.max(...healthData.saude.filter(d => d.peso).map(d => d.peso));
                          const minPeso = Math.min(...healthData.saude.filter(d => d.peso).map(d => d.peso));
                          const altura = peso ? ((peso - minPeso) / (maxPeso - minPeso || 1)) * 100 : 0;
                          return (
                            <div key={index} className="flex-1 flex items-end">
                              <div 
                                ...
                                style={{ height: `${Math.max(altura, 15)}%` }}
                                title={peso ? `${peso}kg - ${new Date(data.data_medicao).toLocaleDateString('pt-BR')}` : 'Sem dados'}
                              />
                            </div>
                          );
                        })}
                  ...
                  {healthData.saude.some(d => d.glicemia_jejum) && (
                    <div className="mb-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-gray-600">Glicemia (mg/dL)</span>
                        <span className="text-sm text-red-600 font-bold">
                          {healthData.saude.find(d => d.glicemia_jejum)?.glicemia_jejum}mg/dL
                        </span>
                      </div>
                      <div className="flex gap-1 h-8">
                        {healthData.saude.slice(0, 7).reverse().map((data, index) => {
                          const glicemia = data.glicemia_jejum;
                          const altura = glicemia ? Math.min((glicemia / 140) * 100, 100) : 0;
                          const cor = glicemia > 100 ? 'bg-red-400' : glicemia > 95 ? 'bg-yellow-400' : 'bg-green-400';
                          return (
                            <div key={index} className="flex-1 flex items-end">
                              <div 
                                className={`w-full rounded-t transition-all duration-300 ${
                                ...
                                style={{ height: `${Math.max(altura, 15)}%` }}
                                title={glicemia ? `${glicemia}mg/dL - ${new Date(data.data_medicao).toLocaleDateString('pt-BR')}` : 'Sem dados'}
                              />
                            </div>
                          );
                        })}
                  ...
                  {healthData.bem_estar && healthData.bem_estar.length > 0 && (
                    <div>
                      <div className="text-sm font-medium text-gray-600 mb-3 text-center">Estado Atual</div>
                      <div className="flex justify-center gap-6">
                        {healthData.bem_estar[0].nivel_estresse !== null && (
                          <div className="text-center">
                            <div className="w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-lg relative overflow-hidden">
                              ...
                              <div 
                                className={`absolute bottom-0 left-0 right-0 transition-all duration-500 ${
                                  healthData.bem_estar[0].nivel_estresse > 7 ? 'bg-red-400' :
                                  healthData.bem_estar[0].nivel_estresse > 4 ? 'bg-yellow-400' : 'bg-green-400'
                                }`}
                                style={{ height: `${(healthData.bem_estar[0].nivel_estresse / 10) * 100}%` }}
                              ></div>
                              <span className="relative z-10 text-gray-700">{healthData.bem_estar[0].nivel_estresse}</span>
                            </div>
                            <div className="text-sm text-gray-600 mt-2">Estresse</div>
                          </div>
                        )}
                        
                        {healthData.bem_estar[0].nivel_energia !== null && (
                          <div className="text-center">
                            <div className="w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-lg relative overflow-hidden">
                              <div className="absolute inset-0 bg-gray-200 rounded-full"></div>
                              <div 
                                className={`absolute bottom-0 left-0 right-0 transition-all duration-500 ${
                                  healthData.bem_estar[0].nivel_energia > 7 ? 'bg-green-400' :
                                  healthData.bem_estar[0].nivel_energia > 4 ? 'bg-yellow-400' : 'bg-red-400'
                                }`}
                                style={{ height: `${(healthData.bem_estar[0].nivel_energia / 10) * 100}%` }}
                              ></div>
                              <span className="relative z-10 text-gray-700">{healthData.bem_estar[0].nivel_energia}</span>
                            </div>
                            <div className="text-sm text-gray-600 mt-2">Energia</div>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {/* Métricas de Saúde */}
            {healthData && healthData.saude && healthData.saude.length > 0 && user && (
              <div className="mb-6">
                <h4 className="text-lg font-bold text-gray-700 mb-4 text-center">Análise de Saúde</h4>
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg space-y-4">
                  {(() => {
                    ...
                    const pesoIdeal = calcularPesoIdeal(user.altura, user.sexo);
                    const progressoPeso = calcularProgressoPeso(dadoAtual.peso, user.peso_inicial, pesoIdeal);
                    const pressaoClass = classificarPressao(dadoAtual.pressao_sistolica, dadoAtual.pressao_diastolica);
                    const glicemiaClass = classificarGlicemia(dadoAtual.glicemia_jejum);
                    ...
                    return (
                      <>
                        ...
                        {imc && (
                          <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white p-3 rounded text-center">
                              <div className="text-xl font-bold text-purple-600">{imc}</div>
                              <div className="text-sm text-gray-600">IMC</div>
                              <div className={`text-sm font-medium ${
                                imcClass.cor === 'green' ? 'text-green-600' :
                                imcClass.cor === 'yellow' ? 'text-yellow-600' :
                                imcClass.cor === 'orange' ? 'text-orange-600' :
                                imcClass.cor === 'red' ? 'text-red-600' : 'text-blue-600'
                              }`}>
                                {imcClass.categoria}
                              </div>
                            </div>
                            ...
                            {pesoIdeal && (
                              <div className="bg-white p-3 rounded text-center">
                                <div className="text-xl font-bold text-pink-600">{pesoIdeal}kg</div>
                                <div className="text-sm text-gray-600">Peso Ideal</div>
                                {progressoPeso && (
                                  <div className="text-sm font-medium text-pink-600">
                                    {progressoPeso > 0 ? `${progressoPeso}% alcançado` : 'Iniciando jornada'}
                                  </div>
                                )}
                              </div>
                            )}
                        ...
                        {/* Pressão Arterial */}
                        {dadoAtual.pressao_sistolica && dadoAtual.pressao_diastolica && (
                          <div className="bg-white p-3 rounded">
                            <div className="flex justify-between items-center">
                              <div>
                                <div className="text-base font-bold text-gray-700">Pressão Arterial</div>
                                <div className="text-xl font-bold text-red-600">
                                  {dadoAtual.pressao_sistolica}/{dadoAtual.pressao_diastolica} mmHg
                                </div>
                              </div>
                              <div className={`px-3 py-2 rounded text-sm font-medium ${
                                pressaoClass.cor === 'green' ? 'bg-green-100 text-green-800' :
                                pressaoClass.cor === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
                                pressaoClass.cor === 'orange' ? 'bg-orange-100 text-orange-800' :
                        ...
                        {dadoAtual.glicemia_jejum && (
                          <div className="bg-white p-3 rounded">
                            <div className="flex justify-between items-center">
                              <div>
                                <div className="text-base font-bold text-gray-700">Glicemia em Jejum</div>
                                <div className="text-xl font-bold text-blue-600">
                                  {dadoAtual.glicemia_jejum} mg/dL
                                </div>
                              </div>
                              <div className={`px-3 py-2 rounded text-sm font-medium ${
                                glicemiaClass.cor === 'green' ? 'bg-green-100 text-green-800' :
                                glicemiaClass.cor === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
                                'bg-red-100 text-red-800'
                              }`}>
                        ...
                        {progressoPeso && progressoPeso > 0 && (
                          <div className="bg-white p-3 rounded">
                            <div className="text-base font-bold text-gray-700 mb-2">Progresso do Peso</div>
                            ...
                            <div className="text-sm text-gray-600 mt-2">
                              {dadoAtual.peso}kg → {pesoIdeal}kg ({progressoPeso}% concluído)
                            </div>
                          </div>
                        )}
                      </>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-blue-600">{healthData?.saude?.length || 0}</div>
                <div className="text-base text-blue-800">Registros de Saúde</div>
              </div>
              <div className="bg-green-50 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-green-600">{medications?.length || 0}</div>
            ...
            <div className="mb-6">
              <h4 className="text-xl font-bold text-gray-800 mb-3">Últimos Registros</h4>
              <div className="bg-gray-50 p-4 rounded-lg">
                {healthData && healthData.saude && healthData.saude.length > 0 ? (
                    ...
                    {healthData.saude.slice(0, 3).map((data, index) => (
                      <div key={index} className="flex justify-between items-center text-base">
                        ...
                        <div className="flex gap-3">
                          ...
                          {data.pressao_sistolica && data.pressao_diastolica && (
                          ...
                          {data.glicemia_jejum && (
                            <span className={`px-3 py-1 rounded text-sm ${
                              data.glicemia_jejum > 100 ? 'bg-red-100 text-red-800' : 
                              data.glicemia_jejum > 95 ? 'bg-yellow-100 text-yellow-800' : 
            ...
            {/* Bem-estar Recente */}
            {healthData && healthData.bem_estar && healthData.bem_estar.length > 0 && (
              <div className="mb-6">
                <h4 className="text-xl font-bold text-gray-800 mb-3">Estado Atual</h4>
                <div className="bg-gray-50 p-4 rounded-lg">
                  {healthData.bem_estar.slice(0, 1).map((bem_estar, index) => (
                    <div key={index} className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {bem_estar.nivel_estresse !== null && (
                          <span className={`px-3 py-2 rounded text-sm ${
                            bem_estar.nivel_estresse > 7 ? 'bg-red-100 text-red-800' :
                            bem_estar.nivel_estresse > 4 ? 'bg-yellow-100 text-yellow-800' :
                            'bg-green-100 text-green-800'
                          }`}>
                        ...
                        {bem_estar.nivel_energia !== null && (
                          <span className={`px-3 py-2 rounded text-sm ${
                            bem_estar.nivel_energia > 7 ? 'bg-green-100 text-green-800' :
                            bem_estar.nivel_energia > 4 ? 'bg-yellow-100 text-yellow-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                      ...
                      {/* Sintomas Físicos */}
                      {bem_estar.sintomas_fisicos && bem_estar.sintomas_fisicos.length > 0 && (
                        <div>
                          <div className="text-sm font-medium text-gray-600 mb-2">Sintomas:</div>
                          <div className="flex flex-wrap gap-2">
                            {bem_estar.sintomas_fisicos.map((sintoma, sintomaIndex) => (
                              <span key={sintomaIndex} className="bg-orange-100 text-orange-800 px-3 py-2 rounded text-sm">
                                ⚠️ {sintoma}
                              </span>
                            ))}
                          </div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className={`absolute bottom-full right-4 mb-2 w-20 h-20 rounded-full flex items-center justify-center transition-all duration-200 shadow-lg ${
                isRecording ? 'bg-red-500 text-white scale-110' : 'bg-white text-teal-500 hover:bg-gray-100'
              } disabled:opacity-50`}
            >
